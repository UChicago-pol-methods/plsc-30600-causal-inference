...
```{r map example}
set.seed(60615) # Set random seed
niter <- 100000 # Number of for loop iterations - this can be as big as we need for precisio
# Pipe %>% makes whatever is piped the first argument in the next function - in this case, 
# just a vector 1:100000 - it ends up not mattering since the actual values of the function 
# aren't used in the simulation - it just defines the length and number of iterations
...
sim_results = 1:niter %>% map_dbl(function(x) var_unadj(rnorm(n=N, mean=mu, sd=sqrt(sigma))))

# Calculate bias
mean(sim_results) - sigma
